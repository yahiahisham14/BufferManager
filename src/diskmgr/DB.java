...
public class DB implements GlobalConst
{
			...
			InvalidPageNumberException, FileIOException, DiskMgrException
	{
		...
		PageId pageId = new PageId();
		pageId.pid = 0;
		pinPage(pageId, apage, true /* no diskIO */, false);

		DBFirstPage firstpg = new DBFirstPage(apage);

		firstpg.setNumDBPages(num_pages);
		unpinPage(pageId, true /* dirty */, false);
...
//		System.out.println("2bl??????");
		...
		{// start forloop01
			...
			Page apage = new Page();
			pinPage(pgid, apage, false /* read disk */, false);
			...
			unpinPage(pgid, false /* undirty */, false);
			...
			OutOfSpaceException, FileIOException, IOException, DiskMgrException
	{
		...
		{// Start DO01
			...
			// Pin the header page
			pinPage(hpid, apage, false /* read disk */, false);
			...
			else if (nexthpid.pid != INVALID_PAGE)
			{
				...
				unpinPage(hpid, false /* undirty */, false);
			}
		...
		if (!found)
		{
			...
			unpinPage(hpid, true /* dirty */, false);

			// Pin the newly-allocated directory page.
			hpid.pid = nexthpid.pid;

			pinPage(hpid, apage, true/* no diskIO */, false);
			...
			FileIOException, InvalidPageNumberException, DiskMgrException
	{
		...
		{// Start DO01
			...
			// Pin the header page.
			pinPage(hpid, apage, false /* no diskIO */, false);
	...
	/**
	 * Set run-size bits starting from start to value specified
	 */
	private void set_bits(PageId start_page, int run_size, int bit/* 1 */)
			throws InvalidPageNumberException, FileIOException, IOException,
			DiskMgrException
	{
		...
		{// Start forloop01
			...
			pinPage(pgid, pg, false/* no diskIO */, false);
			...
			unpinPage(pgid, true /* dirty */, false);
	...
	private void pinPage(PageId pageno, Page page, boolean emptyPage,
			boolean loved) throws DiskMgrException
	{
	 ...
	 */
	private void unpinPage(PageId pageno, boolean dirty, boolean loved)
...
class DBHeaderPage implements PageUsedBytes, GlobalConst
{
	...
	// 0000 0000 000000000.....0
	// page id no. of entries entries
	protected byte[] data;
